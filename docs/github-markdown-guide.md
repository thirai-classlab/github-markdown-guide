...
[D2](https://d2lang.com/) は宣言的なダイアグラム記述言語ですが、**GitHub Markdownではネイティブ非対応**です。
...
上記のように記述しても、GitHub上ではただのコードブロックとして表示され、ダイアグラムにはレンダリングされません。

GitHubがネイティブ対応しているダイアグラム言語は **Mermaid のみ** です（2024年時点）。

#### D2をGitHubで使う代替手段

| 方法 | 概要 |
|------|------|
| **GitHub Action** | [terrastruct/d2-action](https://github.com/terrastruct/d2-action) でCI時にSVG/PNGを自動生成し、画像として埋め込む |
| **プレレンダリング** | ローカルで `d2 input.d2 output.svg` → SVGをコミットして `![diagram](output.svg)` で表示 |
| **VS Code拡張** | ローカルプレビューのみ。GitHub上では効果なし |

#### その他の非対応ダイアグラム言語

以下のダイアグラム言語もGitHub Markdownではネイティブレンダリングされません:

| 言語 | 概要 |
|------|------|
| PlantUML | UML図の記述言語 |
| Graphviz (DOT) | グラフ記述言語 |
| Ditaa | ASCIIアートからダイアグラムを生成 |
| Nomnoml | UMLクラス図の記述 |
| WaveDrom | デジタルタイミング図 |
| Pikchr | PIC風ダイアグラム記述言語 |

> [!TIP]
> これらを使いたい場合は、GitHub Actionやプレレンダリングで画像に変換してからコミットする方法が一般的です。
